...
// --- LISTA DE ESTUDANTES INTELIGENTE (ADAPTA AS COLUNAS) ---
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs border-b sticky top-0 bg-slate-50 z-10">
            <tr>
              ...
              {/* Colunas Dinâmicas baseadas no Filtro */}
              {filterType === 'NEE' && <th className="px-6 py-4">Necessidade (NEE)</th>}
              {filterType === 'NEE' && <th className="px-6 py-4">Idade</th>}
              {filterType === 'CT' && <th className="px-6 py-4">Conselho Tutelar</th>}
              {filterType === 'CT' && <th className="px-6 py-4">Data Enc.</th>}
              {filterType === 'CT' && <th className="px-6 py-4">Motivo</th>}
              
              {filterType !== 'NEE' && filterType !== 'CT' && <th className="px-6 py-4">Turma</th>}
              {filterType !== 'NEE' && filterType !== 'CT' && <th className="px-6 py-4">Status / Info</th>}
          ...
          <tbody className="divide-y divide-slate-100">
            {students.map((s: any) => (
              <tr key={s.id} onClick={() => onSelectStudent(s)} className="hover:bg-indigo-50 cursor-pointer transition-colors group">
                ...
                {/* Renderização Condicional NEE */}
                {filterType === 'NEE' && (
                  <>
                    <td className="px-6 py-4"><span className="bg-purple-100 text-purple-700 px-2 py-1 rounded-md font-bold text-xs">{s.nee_description || 'Não especificado'}</span></td>
                    <td className="px-6 py-4 text-slate-600 font-bold">{getAge(s.birth_date)}</td>
                  </>
                )}

                {/* Renderização Condicional CT */}
                {filterType === 'CT' && (
                  <>
                    <td className="px-6 py-4 text-orange-700 font-bold text-xs uppercase">{s.ct_council_name || 'Não inf.'}</td>
                    <td className="px-6 py-4 text-slate-600 text-xs">{s.ct_date ? new Date(s.ct_date).toLocaleDateString() : '-'}</td>
                    <td className="px-6 py-4 text-slate-500 text-xs italic truncate max-w-[200px]">{s.ct_referral}</td>
                  </>
                )}

                {/* Renderização Padrão */}
                {filterType !== 'NEE' && filterType !== 'CT' && (
                  <>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2 flex-wrap">
                        ...
                        {s.nee_description && <span className="px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-purple-100 text-purple-700 flex items-center gap-1"><Puzzle size={10}/> NEE</span>}
                        {s.ct_referral && <span className="px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-orange-100 text-orange-700 flex items-center gap-1"><Scale size={10}/> CT</span>}
                      </div>
                    </td>
                  </>
                )}
...
export default function App() {
  ...
  const [praisedStudents, setPraisedStudents] = useState<string[]>([]);
  ...
  // --- PERSISTÊNCIA DE DESTAQUE/ELOGIO (BANCO) ---
  ...
  // --- IMPORTAÇÃO V8.0 (Trator + Mapeamento CT) ---
      ...
      try { 
        ...
        for (const row of (data as any[])) {
          ...
          if (aluno) { 
            ...
            // MAPEAR CAMPOS NOVOS DE CT
            if (rowMap['CONSELHO TUTELAR'] || rowMap['CT']) {
                updates.ct_council_name = rowMap['CONSELHO TUTELAR'] || rowMap['CT']; // Qual Conselho
                updates.ct_referral = rowMap['MOTIVO DO ENCAMINHAMENTO'] || rowMap['MOTIVO']; // Motivo
  ...
  // --- A SUPER ATA (AGORA COMPLETA COM TUDO O QUE VOCÊ PEDIU) ---
  const generateSuperAta = (targetClass: string) => {
    ...
    // --- SEÇÃO DE REGISTROS NOMINAIS (O PULO DO GATO) ---
    doc.addPage();
    doc.setFontSize(14); doc.setTextColor(0); doc.text("REGISTROS ESPECÍFICOS E ENCAMINHAMENTOS", 14, 20);
    let currentY = 30;

    const createSection = (title: string, data: any[], color: [number, number, number], headers: string[][]) => {
        if (data.length > 0) {
            doc.setFillColor(...color); doc.rect(14, currentY, 269, 7, 'F');
            doc.setFontSize(10); doc.setTextColor(...color); doc.text(title, 16, currentY + 5);
            autoTable(doc, { startY: currentY + 8, head: headers, body: data, theme: 'grid', styles: { fontSize: 8 }, headStyles: { fillColor: color } });
    ...
    createSection("⭐ DESTAQUES ACADÊMICOS (Certificado)", destaques.map(s => [s.name]), [234, 88, 12], [['Nome']]);
    // 2. ELOGIOS
    const elogios = councilStudents.filter(s => s.is_praised);
    createSection("👏 ELOGIOS E INCENTIVOS", elogios.map(s => [s.name]), [22, 163, 74], [['Nome']]);
    // 3. BAIXO RENDIMENTO (>3)
    ...
    createSection("📉 BAIXO RENDIMENTO (> 3 Notas Vermelhas)", baixoRendimento.map(s => [s.name]), [194, 65, 12], [['Nome']]);
    // 4. BUSCA ATIVA
    ...
    createSection("🚨 BUSCA ATIVA (Faltas > 20)", faltosos.map(s => [s.name, s.desempenho?.find((x:any) => x.bimestre === selectedBimestre)?.faltas_bimestre]), [220, 38, 38], [['Nome', 'Total Faltas']]);
    
    // 5. DELIBERAÇÕES FINAIS (O que você escreveu no modal)
    ...
    if(deliberacoes.length > 0) { 
        doc.addPage(); 
        doc.setFontSize(14); doc.setTextColor(0); doc.text("DELIBERAÇÕES E OBSERVAÇÕES FINAIS", 14, 20); 
        const rowsDelib = deliberacoes.map(s => { const d = s.desempenho?.find((x:any) => x.bimestre === selectedBimestre); return [s.name, d.obs_conselho || '', d.encaminhamento_conselho || '']; }); 
        autoTable(doc, { startY: 25, head: [['Estudante', 'Anotações', 'Encaminhamento']], body: rowsDelib, theme: 'grid', styles: { fontSize: 9, cellWidth: 'wrap' }, columnStyles: { 0: {cellWidth: 60}, 1: { cellWidth: 100 }, 2: { cellWidth: 100 } } }); 
    }

    doc.save(`ATA_COMPLETA_${targetClass}.pdf`);
  };

  // IMPRESSÃO LISTA CONSELHO TUTELAR (Botão Exclusivo)
  const printCTList = () => {
  ...
  const printStudentData = (doc: jsPDF, student: any) => { doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.text("GOVERNO DO DISTRITO FEDERAL", 105, 15, { align: "center" }); doc.text("CED 04 DO GUARÁ - SOE", 105, 22, { align: "center" }); doc.setLineWidth(0.5); doc.line(14, 25, 196, 25); doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.text(`Aluno: ${student.name}`, 14, 35); const logsData = student.logs?.filter((l: any) => l.student_id === student.id).map((l: any) => [new Date(l.created_at).toLocaleDateString(), l.category, JSON.parse(l.description).obs]) || []; autoTable(doc, { startY: 45, head: [['Data', 'Tipo', 'Detalhes']], body: logsData }); doc.save(`Ficha_${selectedStudent.name}.pdf`); };
  ...
  // LISTA INTELIGENTE (FILTRA POR NEE OU CT SE NECESSÁRIO)
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-[#1E1E2D] text-white flex flex-col shadow-2xl transition-transform duration-300 md:static md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 bg-[#151521] border-t border-white/5">
          ...
          <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-slate-500"><div className="flex items-center gap-1"><Code size={10}/> <span className="text-[8px] font-bold uppercase">Dev: Daniel Alves da Silva</span></div><span className="text-[8px]">v8.0 Final</span></div>
        </div>
      </aside>
      ...
      {isModalOpen && selectedStudent && (<div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-[90vw] h-[95vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200"><div className="px-8 py-6 border-b flex justify-between items-center bg-slate-50 flex-shrink-0"><div className="flex items-center gap-6"><div className="relative group"><Avatar name={selectedStudent.name} src={selectedStudent.photo_url} size="xl" /><label className="absolute inset-0 flex items-center justify-center bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"><Camera size={24} /><input type="file" className="hidden" accept="image/*" onChange={handlePhotoUpload} /></label></div><div><h2 className="text-3xl font-bold text-slate-800">{selectedStudent.name}</h2><p className="text-lg text-slate-500 font-bold uppercase mt-1">Turma {selectedStudent.class_id}</p></div></div><div className="flex gap-2"><button onClick={generatePDF} className="p-3 bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200" title="Gerar PDF"><FileDown size={20} /></button><button onClick={startEditing} className="p-3 bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200" title="Editar"><Pencil size={20} /></button><button onClick={() => setIsExitModalOpen(true)} className="p-3 bg-red-100 text-red-600 rounded-full hover:bg-red-200" title="Saída"><LogOut size={20} /></button><button onClick={() => setIsModalOpen(false)} className="ml-4 hover:bg-slate-200 p-2 rounded-full"><X className="text-slate-400 hover:text-red-500" size={32} /></button></div></div><div className="flex border-b px-8 bg-white overflow-x-auto gap-8">{['perfil', 'academico', 'historico', 'familia'].map((tab) => (<button key={tab} onClick={() => setActiveTab(tab as any)} className={`py-5 font-bold text-sm border-b-4 uppercase tracking-wide transition-colors ${activeTab === tab ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>{tab === 'familia' ? 'Família & Responsáveis' : tab.toUpperCase()}</button>))}</div><div className="flex-1 overflow-y-auto p-8 bg-slate-50/50">
        {/* BANNER DE ALERTA NEE/CT NO MODAL */}
                ...
                {selectedStudent.nee_description && (
                    <div className="bg-purple-50 border border-purple-100 p-4 rounded-xl flex items-start gap-3">
                        ...
                        <div>
                            ...
                            {selectedStudent.birth_date && <p className="text-xs text-purple-500 mt-1 font-bold">Idade Calculada: {new Date().getFullYear() - new Date(selectedStudent.birth_date).getFullYear()} anos</p>}
                        </div>
                    </div>
                )}
