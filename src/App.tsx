...
export default function App() {
  ...
  // Admin Photo (Salva no navegador)
  ...
  async function handleSaveLog() {
    ...
    if (error) alert('Erro: ' + error.message);
      ...
      setMotivosSelecionados([]); setAcoesSelecionadas([]); setObsLivre(DEFAULT_OBS); // Reseta para o template
      fetchStudents();
    }
  }
  ...
  const handleLogout = () => {
    // Simples reload para "sair" por enquanto
    ...
    <div className="flex h-screen bg-slate-100 font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="bg-white border-b px-8 py-4 flex justify-between items-center shadow-sm z-10">
          ...
          {/* HEADER DO USUÁRIO COM UPLOAD DE FOTO */}
