...
// --- LISTA RÁPIDA AMPLIADA (MODO FLASH) ---
const FLASH_REASONS = [
    "Uniforme Inadequado", "Atraso / Chegada Tardia", "Uso de Celular", 
    "Sem Material", "Saída de Sala", "Conversa / Bagunça",
    "Conflito entre Colegas", "Sono em Sala", "Falta de Atividade",
    "Elogio / Destaque Positivo", "Encaminhamento Saúde", "Outros"
];
...
export default function App() {
  ...
  const generatePDF = () => {
    ...
    if (selectedStudent.logs && selectedStudent.logs.length > 0) {
      ...
      sortedLogs.forEach((log) => {
        ...
        if(log.category === 'Família') { doc.setTextColor(234, 88, 12); doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.text(`[FAMÍLIA] ${dataAtendimento} - Solicitante: ${parsed.solicitante || 'SOE'}`, 20, currentY); } 
        else { doc.setTextColor(0, 0, 0); doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.text(`${dataAtendimento} - Solicitante: ${parsed.solicitante || 'SOE'}`, 20, currentY); }
        doc.setTextColor(0); doc.setFont("helvetica", "normal"); 
        let textoFinal = (parsed.motivos && parsed.motivos.length > 0) ? `[${parsed.motivos.join(', ')}] ${parsed.obs}` : parsed.obs;
        const splitObs = doc.splitTextToSize(`Relato: ${textoFinal}`, 170); doc.text(splitObs, 20, currentY + 5);
        ...
        if (log.return_date) { const dataRetorno = new Date(log.return_date).toLocaleDateString('pt-BR', {timeZone: 'UTC'}); doc.setFont("helvetica", "bold"); doc.setTextColor(220, 38, 38); doc.text(`Retorno Agendado: ${dataRetorno}`, 120, currentY - 6); doc.setTextColor(0); }
      ...
      <div className="space-y-6 h-full flex flex-col">
        ...
        {sortedReturns.length > 0 && (
           <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex flex-col gap-3">
             <div className="flex items-center gap-2 text-amber-800 font-bold uppercase text-sm"><Clock size={18} /><h3>Agendamentos de Retorno ({sortedReturns.length})</h3></div>
                 ...
                 return (
                   <div key={idx} className={`min-w-[200px] p-3 rounded-xl border shadow-sm bg-white ${isLate ? 'border-red-300' : 'border-amber-200'}`}>
                      <div className="flex justify-between items-start mb-1">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded ${isLate ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>{new Date(ret.return_date!).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 overflow-hidden min-h-0">
          <div className="bg-white rounded-2xl border border-red-100 shadow-sm flex flex-col h-full overflow-hidden">
            <div className="bg-red-50 px-6 py-4 border-b border-red-100 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2"><AlertCircle className="text-red-600" size={20} /><div><h3 className="font-bold text-red-800 uppercase">Alunos em Alerta {selectedClassFilter && `(${selectedClassFilter})`}</h3>{selectedClassFilter && <span className="text-[10px] text-red-600 cursor-pointer underline" onClick={() => setSelectedClassFilter(null)}>Limpar Filtro</span>}</div></div>
          ...
          <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm flex flex-col h-full overflow-hidden">
            <div className="bg-indigo-50 px-6 py-4 border-b border-indigo-100 flex items-center justify-between flex-shrink-0"><div className="flex items-center gap-2"><BarChart3 className="text-indigo-600" size={20} /><h3 className="font-bold text-indigo-800 uppercase">Estatísticas por Turma</h3></div><p className="text-[10px] text-indigo-400">Toque para filtrar</p></div>
        ...
        <button onClick={() => { setQuickSearchTerm(''); setQuickSelectedStudent(null); setQuickReason(''); setIsQuickModalOpen(true); }} className="fixed bottom-6 right-6 w-14 h-14 bg-indigo-600 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-40 border-4 border-white"><Zap size={24} fill="white" /></button>
      </div>
    );
  };
  ...
  async function saveEdits() {
    if (!selectedStudent) return;
    const { error } = await supabase.from('students').update({ name: editName, class_id: editClass, guardian_name: editGuardian, guardian_phone: editPhone, address: editAddress, absences: editAbsences, performance: editPerformance, grades: editGrades }).eq('id', selectedStudent.id);
  ...
  async function handleSaveLog() {
    ...
    const { error } = await supabase.from('logs').insert([{ student_id: selectedStudent.id, category: currentCategory, description: desc, referral: encaminhamento, resolved: resolvido, created_at: selectedDateISO, return_date: returnDate || null }]);
    if (error) alert('Erro: ' + error.message);
    else { alert(`Salvo em ${currentCategory}!`); setMotivosSelecionados([]); setObsLivre(DEFAULT_OBS); setAttendanceDate(new Date().toISOString().split('T')[0]); setReturnDate(''); fetchStudents(); setIsModalOpen(false); }
  }
  ...
  async function handleQuickSave() {
    ...
    if (error) alert('Erro: ' + error.message);
    else { alert(`Registro rápido salvo!`); setIsQuickModalOpen(false); fetchStudents(); }
  }
  ...
  function handleAdminPhotoUpload(event: React.ChangeEvent<HTMLInputElement>) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e) => { const result = e.target?.result as string; setAdminPhoto(result); localStorage.setItem('adminPhoto', result); };
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans text-slate-800 relative">
      <style>{`
        @media print { aside, header, .no-print { display: none !important; } body { background: white; } .modal-content { box-shadow: none !important; width: 100% !important; max-width: 100% !important; height: auto !important; position: relative !important; overflow: visible !important; } .modal-overlay { position: static !important; background: white !important; padding: 0 !important; } .new-log-area, .tabs-header { display: none !important; } .print-header { display: block !important; } .tab-content { display: block !important; } }
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-slate-900 text-white flex flex-col shadow-2xl transition-transform duration-300 ease-in-out md:static md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex items-center gap-3 border-b border-slate-800"><div className="bg-indigo-600 p-2 rounded-lg"><BookOpen size={20} className="text-white"/></div><div><h1 className="font-bold text-lg">SOE Digital</h1><p className="text-[10px] uppercase text-slate-400">CED 4 Guará</p></div></div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full">
        <header className="bg-white border-b px-4 md:px-8 py-3 flex justify-between items-center shadow-sm z-10 flex-shrink-0 gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1 max-w-md relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input type="text" placeholder="Buscar aluno em toda a escola..." className="w-full pl-10 pr-4 py-2 bg-slate-100 border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500 rounded-full text-sm transition-all outline-none" value={globalSearch} onChange={(e) => { setGlobalSearch(e.target.value); if(e.target.value.length > 0) setView('students'); }} /></div>
          </div>
        ...
        <div className="modal-overlay fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="modal-content bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[95vh] flex flex-col overflow-hidden">
            ...
            <div className="px-4 md:px-8 py-4 md:py-6 border-b flex justify-between items-center bg-slate-50 no-print flex-shrink-0">
              <div className="flex items-center gap-4"><div className="relative group hidden md:block"><Avatar name={selectedStudent.name} src={selectedStudent.photo_url} size="lg" /><label className="absolute inset-0 flex items-center justify-center bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"><Camera size={20} /><input type="file" className="hidden" accept="image/*" onChange={handlePhotoUpload}/></label></div><div><h2 className="text-lg md:text-2xl font-bold text-slate-800 line-clamp-1">{selectedStudent.name}</h2><p className="text-xs md:text-sm text-slate-500 font-bold uppercase">Turma {selectedStudent.class_id}</p></div></div>
              <div className="flex items-center gap-2"><button onClick={generatePDF} className="p-2 bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200" title="Gerar PDF Oficial"><FileDown size={18} /></button><button onClick={startEditing} className="p-2 bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200" title="Editar"><Pencil size={18} /></button><button onClick={() => setIsExitModalOpen(true)} className="p-2 bg-red-100 text-red-600 rounded-full hover:bg-red-200" title="Registrar Saída"><LogOut size={18} /></button><button onClick={() => setIsModalOpen(false)}><X className="text-slate-400 hover:text-red-500" size={28}/></button></div>
            </div>
            <div className="flex border-b px-4 md:px-8 bg-white overflow-x-auto no-print flex-shrink-0"><button onClick={() => setActiveTab('perfil')} className={`px-4 md:px-6 py-4 font-bold text-xs md:text-sm border-b-2 whitespace-nowrap ${activeTab === 'perfil' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500'}`}>DADOS PESSOAIS</button><button onClick={() => setActiveTab('academico')} className={`px-4 md:px-6 py-4 font-bold text-xs md:text-sm border-b-2 whitespace-nowrap ${activeTab === 'academico' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500'}`}>BOLETIM</button><button onClick={() => setActiveTab('historico')} className={`px-4 md:px-6 py-4 font-bold text-xs md:text-sm border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'historico' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500'}`}><UserCircle size={16}/> ATENDIMENTOS ESTUDANTE</button><button onClick={() => setActiveTab('familia')} className={`px-4 md:px-6 py-4 font-bold text-xs md:text-sm border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'familia' ? 'border-orange-500 text-orange-600' : 'border-transparent text-slate-500'}`}><Users2 size={16}/> FAMÍLIA / RESPONSÁVEIS</button></div>
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50">
              {activeTab === 'perfil' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 tab-content">
                  <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden"><div className="bg-indigo-50 px-6 py-4 border-b border-indigo-100 flex items-center gap-2"><UserCircle className="text-indigo-600" /><h3 className="font-bold text-indigo-900 uppercase">Informações de Contato</h3></div><div className="p-6 space-y-6"><div className="flex items-start gap-4"><div className="bg-slate-100 p-2 rounded-lg"><User size={20} className="text-slate-500"/></div><div className="flex-1"><span className="block text-xs font-bold text-slate-400 uppercase mb-1">Responsável Legal</span>{isEditing ? <input value={editGuardian} onChange={e=>setEditGuardian(e.target.value)} className="w-full border rounded p-2"/> : <p className="font-medium text-lg text-slate-800 break-words">{selectedStudent.guardian_name || "Não informado"}</p>}</div></div><div className="flex items-start gap-4"><div className="bg-slate-100 p-2 rounded-lg"><Phone size={20} className="text-slate-500"/></div><div className="flex-1"><span className="block text-xs font-bold text-slate-400 uppercase mb-1">Telefone / WhatsApp</span>{isEditing ? <input value={editPhone} onChange={e=>setEditPhone(e.target.value)} className="w-full border rounded p-2"/> : <p className="font-medium text-lg text-slate-800 break-words whitespace-pre-line">{selectedStudent.guardian_phone || "Não informado"}</p>}</div></div><div className="flex items-start gap-4"><div className="bg-slate-100 p-2 rounded-lg"><MapPin size={20} className="text-slate-500"/></div><div className="flex-1"><span className="block text-xs font-bold text-slate-400 uppercase mb-1">Endereço Residencial</span>{isEditing ? <input value={editAddress} onChange={e=>setEditAddress(e.target.value)} className="w-full border rounded p-2"/> : <p className="font-medium text-lg text-slate-800 leading-relaxed break-words">{selectedStudent.address || "Não informado"}</p>}</div></div>{isEditing && <button onClick={saveEdits} className="w-full mt-4 bg-green-600 text-white px-4 py-3 rounded-xl font-bold shadow-md hover:bg-green-700 transition-colors">Salvar Alterações</button>}</div></div>
                </div>
              )}
              {activeTab === 'academico' && (
                <div className="bg-white rounded-xl border shadow-sm overflow-x-auto tab-content"><table className="w-full text-sm text-left min-w-[600px]"><thead className="bg-slate-50 text-[10px] font-bold uppercase text-slate-500"><tr><th className="px-6 py-3">Bimestre</th><th className="px-2 py-3">LP</th><th className="px-2 py-3">MAT</th><th className="px-2 py-3">CIE</th><th className="px-2 py-3">HIS</th><th className="px-2 py-3">GEO</th><th className="px-2 py-3">ING</th><th className="px-2 py-3">ART</th><th className="px-2 py-3">EDF</th><th className="px-2 py-3 bg-slate-100">PD1</th><th className="px-2 py-3 bg-slate-100">PD2</th><th className="px-2 py-3 bg-slate-100">PD3</th><th className="px-2 py-3 text-red-600">Faltas</th></tr></thead><tbody className="divide-y divide-slate-100">{selectedStudent.desempenho?.map((d, i) => (<tr key={i} className="hover:bg-slate-50"><td className="px-6 py-4 font-bold">{d.bimestre}</td><td className={`px-2 py-4 font-bold ${d.lp < 5 ? 'text-red-500' : ''}`}>{d.lp}</td><td className={`px-2 py-4 font-bold ${d.mat < 5 ? 'text-red-500' : ''}`}>{d.mat}</td><td className="px-2 py-4">{d.cie}</td><td className="px-2 py-4">{d.his}</td><td className="px-2 py-4">{d.geo}</td><td className="px-2 py-4">{d.ing}</td><td className="px-2 py-4">{d.art}</td><td className="px-2 py-4">{d.edf}</td><td className="px-2 py-4 bg-slate-50">{d.pd1}</td><td className="px-2 py-4 bg-slate-50">{d.pd2}</td><td className="px-2 py-4 bg-slate-50">{d.pd3}</td><td className="px-2 py-4 font-bold text-red-600 bg-red-50 text-center">{d.faltas_bimestre}</td></tr>))}</tbody></table></div>
              )}
              {(activeTab === 'historico' || activeTab === 'familia') && (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 tab-content">
                  <div className="lg:col-span-7 space-y-6 no-print">
                    <div className={`p-6 rounded-xl border shadow-sm new-log-area ${activeTab === 'familia' ? 'bg-orange-50 border-orange-200' : 'bg-white border-indigo-100'}`}><h3 className={`font-bold mb-6 border-b pb-2 uppercase text-sm flex items-center gap-2 ${activeTab === 'familia' ? 'text-orange-800 border-orange-200' : 'text-indigo-800 border-indigo-100'}`}>{activeTab === 'familia' ? <><Users2 size={18}/> Novo Atendimento à Família</> : <><FileText size={18}/> Novo Atendimento ao Estudante</>}</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"><div><label className="text-xs font-bold text-slate-500 uppercase">Solicitante</label><select className="w-full mt-1 p-3 border rounded-lg text-sm bg-white focus:ring-2 transition-colors" value={solicitante} onChange={e => setSolicitante(e.target.value)}><option>Professor</option><option>Coordenação</option><option>Responsável</option><option>Disciplinar</option><option>Próprio Aluno</option><option>Direção</option></select></div><div><label className="text-xs font-bold text-slate-500 uppercase">Encaminhar Para</label><select className="w-full mt-1 p-3 border rounded-lg text-sm bg-white focus:ring-2 transition-colors" value={encaminhamento} onChange={e => setEncaminhamento(e.target.value)}><option value="">-- Selecione --</option>{ENCAMINHAMENTOS.map(e => <option key={e}>{e}</option>)}</select></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"><div><label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2 mb-1"><CalendarDays size={14}/> Data do Atendimento</label><input type="date" className="w-full p-3 border rounded-lg text-sm bg-white focus:ring-2" value={attendanceDate} onChange={e => setAttendanceDate(e.target.value)} /></div><div><label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2 mb-1 text-indigo-600"><Clock size={14}/> Agendar Retorno (Opcional)</label><input type="date" className="w-full p-3 border border-indigo-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-indigo-200" value={returnDate} onChange={e => setReturnDate(e.target.value)} /></div></div><label className="text-xs font-bold text-slate-500 uppercase block mb-3">Motivo do Atendimento</label><div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4 bg-white/50 rounded-xl border border-slate-200"><div><p className="text-[10px] font-bold text-orange-600 uppercase mb-2">Comportamental</p>{MOTIVOS_COMPORTAMENTO.map(m => (<label key={m} className="flex gap-2 text-xs text-slate-600 cursor-pointer mb-1 hover:text-slate-900"><input type="checkbox" checked={motivosSelecionados.includes(m)} onChange={() => toggleItem(motivosSelecionados, setMotivosSelecionados, m)}/> {m}</label>))}</div><div><p className="text-[10px] font-bold text-blue-600 uppercase mb-2">Pedagógico</p>{MOTIVOS_PEDAGOGICO.map(m => (<label key={m} className="flex gap-2 text-xs text-slate-600 cursor-pointer mb-1 hover:text-slate-900"><input type="checkbox" checked={motivosSelecionados.includes(m)} onChange={() => toggleItem(motivosSelecionados, setMotivosSelecionados, m)}/> {m}</label>))}</div><div><p className="text-[10px] font-bold text-purple-600 uppercase mb-2">Social / Outros</p>{MOTIVOS_SOCIAL.map(m => (<label key={m} className="flex gap-2 text-xs text-slate-600 cursor-pointer mb-1 hover:text-slate-900"><input type="checkbox" checked={motivosSelecionados.includes(m)} onChange={() => toggleItem(motivosSelecionados, setMotivosSelecionados, m)}/> {m}</label>))}</div></div><label className="text-xs font-bold text-slate-500 uppercase block mb-2">Relatório Detalhado</label><textarea className="w-full p-4 border rounded-xl bg-white text-sm focus:ring-2 focus:ring-indigo-100 transition-all shadow-inner" rows={10} value={obsLivre} onChange={e => setObsLivre(e.target.value)} /><div className="flex justify-between items-center mt-6 pt-4 border-t border-slate-200"><label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-green-700 select-none p-2 hover:bg-green-50 rounded-lg transition-colors"><input type="checkbox" className="w-5 h-5 rounded text-green-600 focus:ring-green-500" checked={resolvido} onChange={e => setResolvido(e.target.checked)}/> Caso Resolvido / Finalizado</label><button onClick={handleSaveLog} className={`text-white px-8 py-3 rounded-xl font-bold shadow-lg transition-transform active:scale-95 flex items-center gap-2 ${activeTab === 'familia' ? 'bg-orange-600 hover:bg-orange-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}><Save size={18}/> {activeTab === 'familia' ? 'Salvar Contato Família' : 'Salvar Atendimento'}</button></div></div>
                  </div>
                  <div className="lg:col-span-5 bg-slate-100 rounded-xl p-4 overflow-y-auto max-h-[800px] print:col-span-12 print:bg-white print:border print:border-black">
                    <h3 className="text-xs font-bold text-slate-500 uppercase mb-4 print:text-black sticky top-0 bg-slate-100 py-2 z-10">Histórico: {activeTab === 'familia' ? 'Família & Responsáveis' : 'Estudante'}</h3>
                    ...
                    {selectedStudent.logs?.filter(l => activeTab === 'familia' ? l.category === 'Família' : l.category !== 'Família').map(log => {
                        ...
                        return (
                          <div key={log.id} className={`p-5 rounded-xl border shadow-sm mb-4 hover:shadow-md transition-shadow bg-white ${log.category === 'Família' ? 'border-l-4 border-l-orange-400' : 'border-l-4 border-l-indigo-400'}`}>
                            <div className="flex justify-between items-start mb-3 border-b pb-2"><div><span className={`font-bold text-sm block ${log.category === 'Família' ? 'text-orange-700' : 'text-indigo-700'}`}>{dataVisual}</span><span className="text-[10px] text-slate-400 uppercase font-bold">{p.solicitante || 'SOE'}</span></div>{log.resolved ? <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-1 rounded uppercase border border-green-200">Resolvido</span> : <span className="bg-amber-100 text-amber-700 text-[10px] font-bold px-2 py-1 rounded uppercase border border-amber-200">Em Aberto</span>}</div>
                            {p.motivos && p.motivos.length > 0 && (<div className="flex flex-wrap gap-2 mb-3">{p.motivos.map((m: string, i: number) => (<span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] font-bold rounded border border-indigo-100 uppercase">{m}</span>))}</div>)}
                            ...
                            {dataRetorno && !log.resolved && (<div className="mt-2 flex items-center gap-2 text-xs font-bold text-amber-600 uppercase bg-amber-50 p-2 rounded"><Clock size={14}/> Retorno Agendado: {dataRetorno}</div>)}
                          </div>
                        )
                    })}
                  </div>
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6"><h3 className="text-xl font-bold mb-4 text-indigo-600 flex items-center gap-2"><FileSpreadsheet size={24}/> Importar Excel</h3><div className="space-y-4"><div><label className="block text-sm font-bold text-slate-700 mb-1 font-bold">Bimestre de Referência</label><select className="w-full p-3 border rounded-xl" value={selectedBimestre} onChange={e => setSelectedBimestre(e.target.value)}><option>1º Bimestre</option><option>2º Bimestre</option><option>3º Bimestre</option><option>4º Bimestre</option></select></div><div className="border-2 border-dashed border-indigo-200 rounded-xl p-8 text-center bg-indigo-50">{importing ? <p className="animate-pulse font-bold text-indigo-600">Sincronizando iEducar...</p> : <input type="file" accept=".xlsx, .xls" title="Arquivo" onChange={handleFileUpload} className="w-full text-sm"/>}</div><div className="flex justify-end"><button onClick={() => setIsImportModalOpen(false)} className="px-4 py-2 text-slate-500 font-bold">Fechar</button></div></div></div></div>
      )}

      {isExitModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6"><h3 className="text-xl font-bold mb-4 text-red-600">Registrar Saída</h3><div className="space-y-4"><div className="flex gap-4"><label className="flex items-center gap-2 font-bold text-sm"><input type="radio" checked={exitType === 'TRANSFERIDO'} onChange={() => setExitType('TRANSFERIDO')} /> TRANSFERÊNCIA</label><label className="flex items-center gap-2 font-bold text-sm text-red-600"><input type="radio" checked={exitType === 'ABANDONO'} onChange={() => setExitType('ABANDONO')} /> ABANDONO</label></div><textarea className="w-full p-3 border rounded-xl" placeholder="Motivo da saída..." value={exitReason} onChange={e => setExitReason(e.target.value)} /><div className="flex justify-end gap-2"><button onClick={() => setIsExitModalOpen(false)} className="px-4 py-2">CANCELAR</button><button onClick={handleRegisterExit} className="bg-red-600 text-white px-6 py-2 rounded-xl font-bold">CONFIRMAR</button></div></div></div></div>
      )}
      ...
      {isQuickModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="text-center mb-6"><div className="bg-indigo-100 w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3"><Zap size={28} className="text-indigo-600 fill-indigo-600"/></div><h3 className="text-xl font-bold text-slate-800">Registro Rápido</h3><p className="text-xs text-slate-500">Modo simplificado para mobile</p></div>
            <div className="space-y-4">
               <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Quem é o estudante?</label><div className="relative"><input autoFocus placeholder="Digite o nome..." className="w-full p-3 pl-10 border rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none" value={quickSearchTerm} onChange={e => { setQuickSearchTerm(e.target.value); setQuickSelectedStudent(null); }} /><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18}/>
                   {quickSearchTerm.length > 2 && !quickSelectedStudent && (<div className="absolute top-full left-0 right-0 mt-1 bg-white border rounded-xl shadow-xl max-h-48 overflow-y-auto z-50">{students.filter(s => s.name.toLowerCase().includes(quickSearchTerm.toLowerCase())).slice(0, 5).map(s => (<div key={s.id} onClick={() => { setQuickSelectedStudent(s); setQuickSearchTerm(s.name); }} className="p-3 hover:bg-indigo-50 cursor-pointer border-b last:border-0"><p className="font-bold text-sm text-slate-700">{s.name}</p><p className="text-xs text-slate-400">Turma {s.class_id}</p></div>))}</div>)}
                 </div>{quickSelectedStudent && <p className="text-xs text-green-600 font-bold mt-1 text-center">✅ {quickSelectedStudent.name} selecionado</p>}</div>
               <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Qual o motivo?</label><div className="grid grid-cols-2 gap-2">{FLASH_REASONS.map(r => (<button key={r} onClick={() => setQuickReason(r)} className={`p-3 rounded-xl text-xs font-bold transition-all border ${quickReason === r ? 'bg-indigo-600 text-white border-indigo-600 shadow-md transform scale-105' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100'}`}>{r}</button>))}</div></div>
               <button onClick={handleQuickSave} disabled={!quickSelectedStudent || !quickReason} className="w-full py-4 bg-green-600 text-white rounded-xl font-bold shadow-lg hover:bg-green-700 disabled:opacity-50 transition-all mt-4 flex items-center justify-center gap-2"><Save size={20}/> SALVAR REGISTRO</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isNewStudentModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6"><h3 className="text-xl font-bold mb-4">Novo Aluno</h3><form onSubmit={handleAddStudent} className="space-y-4"><input className="w-full p-3 border rounded-xl" placeholder="Nome Completo" value={newName} onChange={e => setNewName(e.target.value)} /><div className="grid grid-cols-2 gap-4"><input className="w-full p-3 border rounded-xl" placeholder="Turma (ex: 6A)" value={newClass} onChange={e => setNewClass(e.target.value)} /><div className="w-full p-3 border rounded-xl bg-slate-100 text-slate-500 flex items-center p-3 text-sm font-bold">Vespertino</div></div><input className="w-full p-3 border rounded-xl" placeholder="Responsável" value={newResponsavel} onChange={e => setNewResponsavel(e.target.value)} /><div className="flex justify-end gap-2 mt-4"><button type="button" onClick={() => setIsNewStudentModalOpen(false)} className="px-4 py-2">Cancelar</button><button type="submit" className="bg-indigo-600 text-white px-6 py-2 rounded-xl font-bold">Salvar</button></div></form></div></div>
      )}
    </div>
  );
}