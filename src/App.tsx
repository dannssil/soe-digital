...
import { 
  ...
  Puzzle, Scale, Cake, Siren, Bell, ListChecks, FileInput, Book, FileSignature, Sparkles, Layers, Loader2, ZoomIn, BellRing 
} from 'lucide-react';
...
export default function App() {
  ...
  const renderConselho = () => {
      ...
      return (
          <div className="max-w-[1800px] mx-auto pb-4 w-full h-full flex flex-col md:overflow-hidden overflow-y-auto bg-slate-50/50">
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center shadow-sm z-10 gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {(() => {
                ...
                const returnsToday = students.flatMap(s => s.logs?.map((l:any) => ({...l, studentName: s.name, studentId: s.id})) || []).filter(l => !l.resolved && l.return_date && l.return_date.split('T')[0] === today);
                return (
                    <div className="relative">
                        <button onClick={() => setIsAlertsOpen(!isAlertsOpen)} className="relative p-2 rounded-full hover:bg-slate-100 text-slate-600 transition-colors">
                            <BellRing size={20}/>
                            {returnsToday.length > 0 && <span className="absolute top-1 right-1 w-4 h-4 bg-red-500 text-white text-[9px] flex items-center justify-center rounded-full font-bold animate-pulse">{returnsToday.length}</span>}
                        </button>
                        {isAlertsOpen && (
                            <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 animate-in zoom-in-50 duration-200">
                                <div className="p-3 border-b bg-slate-50 rounded-t-xl"><h4 className="font-bold text-xs uppercase text-slate-500">Retornos Hoje ({returnsToday.length})</h4></div>
                                <div className="max-h-64 overflow-y-auto p-2">
                                    {returnsToday.length === 0 ? <p className="text-xs text-center text-slate-400 py-4">Nenhum retorno agendado.</p> : 
                                        returnsToday.map((r:any) => (
                                            <div key={r.id} onClick={() => { setSelectedStudent(students.find(s => s.id === r.studentId)); setIsModalOpen(true); setIsAlertsOpen(false); }} className="p-3 hover:bg-indigo-50 rounded-lg cursor-pointer border-b border-slate-50 last:border-0 group">
                                                <p className="font-bold text-sm text-indigo-900 group-hover:text-indigo-600">{r.studentName}</p>
                                                <p className="text-xs text-slate-500 italic truncate">{JSON.parse(r.description).obs}</p>
                                            </div>
                                        ))
                                    }
                                </div>
            ...
            <div className="text-right hidden md:block"><p className="text-xs font-bold text-slate-700">{new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</p></div><div className="bg-indigo-100 text-indigo-700 p-2 rounded-full"><Users2 size={18}/></div>
          </div>
        </header>
      ...
      {/* MODAL DETALHES ALUNO (MOBILE CORRIGIDO) */}
      {isModalOpen && selectedStudent && (
      <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-end md:items-center justify-center p-0 md:p-4 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
        <div className="bg-white rounded-t-2xl md:rounded-2xl shadow-2xl w-full max-w-[90vw] h-[95vh] flex flex-col overflow-hidden">
            
            {/* CABEÇALHO FIXO (STICKY) PARA MOBILE */}
            <div className="sticky top-0 z-50 bg-white px-6 py-4 md:px-8 md:py-6 border-b flex justify-between items-center shadow-sm">
                
                {/* BOTÃO VOLTAR (SÓ MOBILE) */}
                <button onClick={() => setIsModalOpen(false)} className="md:hidden p-2 mr-2 -ml-2 rounded-full text-slate-500 hover:bg-slate-100">
                    <ChevronLeft size={28} />
                </button>
                ...
                <div className="flex items-center gap-4 md:gap-6 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0 overflow-hidden">
                        <h2 className="text-lg md:text-3xl font-bold text-slate-800 truncate leading-tight">{selectedStudent.name}</h2>
                        <p className="text-xs md:text-lg text-slate-500 font-bold uppercase mt-1">Turma {selectedStudent.class_id}</p>
                    </div>
                </div>

                {/* BOTÕES DE AÇÃO (ESCONDIDOS NO MOBILE SE PRECISAR DE ESPAÇO, OU ADAPTADOS) */}
                <div className="flex gap-2 items-center">
                    <button onClick={generatePDF} className="p-2 md:p-3 bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200 flex items-center gap-2" title="Gerar Ficha"><FileDown size={20} /><span className="text-xs font-bold uppercase hidden md:inline">Ficha</span></button>
                    <button onClick={startEditing} className="p-2 md:p-3 bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200" title="Editar"><Pencil size={20} /></button>
                    <button onClick={() => setIsExitModalOpen(true)} className="p-2 md:p-3 bg-red-100 text-red-600 rounded-full hover:bg-red-200" title="Saída"><LogOut size={20} /></button>
                    
                    {/* BOTÃO FECHAR (DESKTOP) */}
                    <button onClick={() => setIsModalOpen(false)} className="hidden md:block ml-4 hover:bg-slate-200 p-2 rounded-full"><X className="text-slate-400 hover:text-red-500" size={32} /></button>
                </div>
            </div>

            {/* BARRA DE NAVEGAÇÃO (ABAS COM ÍCONES) */}
            <div className="flex border-b px-4 md:px-8 bg-white overflow-x-auto gap-4 md:gap-8 scrollbar-thin">
            {['perfil', 'academico', 'historico', 'familia'].map((tab) => {
                let label = tab.toUpperCase();
                let icon = <UserCircle size={18} />;
                if (tab === 'academico') { label = 'ACADÊMICO'; icon = <BookOpen size={18}/>; }
                if (tab === 'historico') { label = 'HISTÓRICO'; icon = <History size={18}/>; }
                if (tab === 'familia') { label = 'Família'; icon = <Users2 size={18}/>; }

                return (
                <button
                    key={tab}
                    onClick={() => setActiveTab(tab as any)}
                    className={`py-4 md:py-5 font-bold text-xs md:text-sm border-b-4 uppercase tracking-wide transition-colors whitespace-nowrap flex items-center gap-2 ${
                    activeTab === tab
            ...
            {/* CONTEÚDO COM SCROLL */}
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50/50 pb-20 md:pb-8">
