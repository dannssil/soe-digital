...
export default function App() {
  ...
  // FUNÇÕES DE HELPERS (DEFINIDAS ANTES DO USO)
  ...
  // --- IMPORTAÇÃO INTELIGENTE V9 (Com limpeza de Turma) ---
    ...
    reader.onload = async (evt) => { 
      try { 
        ...
        for (const row of (data as any[])) {
          ...
          if (aluno) { 
            const updates: any = {};
            // Limpeza de Turma (Ex: "6 A" -> "6A")
            if (rowMap['TURMA']) {
                updates.class_id = rowMap['TURMA'].toString().toUpperCase().replace(/\s/g, '');
            }
            // Data Nascimento
  ...
  // --- A "SUPER ATA" COM IDENTIFICAÇÃO VISUAL CLARA ---
    ...
    return (
      <div className="space-y-6 pb-20 w-full max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[400px]">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col"><h4 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2"><PieChartIcon size={16} className="text-indigo-600"/> Motivos Recorrentes</h4><div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.pieData} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">{stats.pieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Pie><Tooltip contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}}/><Legend layout="horizontal" verticalAlign="bottom" align="center" wrapperStyle={{fontSize: '10px'}}/></PieChart></ResponsiveContainer></div></div>
        </div>
